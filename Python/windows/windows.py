...
class Page_1_1:  # 这是第二个页面
    def __init__(self, window):
        ...
        self.button1 = tk.Button(self.window, text="请选择需要识别的用户",height=3,width=100, command=self.setupUI(root))
        self.tk= tk
        self.button.pack()
        self.button1.pack()
    ...
    def start(self,pythonFile,face_id):
        ...
        tk.messagebox.showinfo(title='系统提示', message='识别成功')
        Page_1(root)
    ...
    def setupUI(self,window):
        ...
        n=len(dirs)
        for n in range(n):
            self.n=tk.Button(self.window,text="%s"%dirs[n],height=3,width=100,command=lambda:self.start("./锁屏.py",dirs[n]))
            self.n.pack()      
